...
def page_index_main(doc, opt=None):
    set_llm_defaults_from_opt(opt)
    logger = JsonLogger(doc)
    
    is_supported_input = (
        (isinstance(doc, str) and os.path.isfile(doc) and doc.lower().endswith((".pdf", ".json"))) or 
        isinstance(doc, BytesIO)
    )
    if not is_supported_input:
        raise ValueError("Unsupported input type. Expected a PDF/JSON file path or PDF BytesIO object.")
    ...
    page_list = get_page_tokens(
        ...
        model=opt.model,
        pdf_parser='PyPDF2',
        page_worker_count=getattr(opt, 'pdf_page_worker_count', 1),
    )
